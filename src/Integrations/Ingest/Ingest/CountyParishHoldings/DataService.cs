...
public class DataService(ICphRepository cphRepository) : IDataService<Identifier>
{
    ...
    {
        var existing = await cphRepository
            .GetSingle(x => x.Identifier.Equals(model.Value), cancellationToken);
        if (existing == null)
        {
            await cphRepository
                .Create(
                    new Postgres.Database.Entities.CountyParishHoldings { Identifier = model.Value },
                    cancellationToken);
        }
    }
}
