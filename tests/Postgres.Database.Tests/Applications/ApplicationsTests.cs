...
namespace Defra.Identity.Postgres.Database.Tests.Applications;
...
public class ApplicationsTests(PostgreContainerFixture fixture) : BaseTests(fixture)
{
    [Fact]
    [Description("Add a new application and check it's been added")]
    public async Task ShouldAddApplication()
    {
        var admin = await Context.UserAccounts.SingleAsync(x => x.EmailAddress == "test@test.com", TestContext.Current.CancellationToken);
