...
public static class DefraLoggerExtensions
{
        ...
        where TException : Exception
    {
        var matchingCalls = logger.ReceivedCalls()
            ...
            .Where(call => (LogLevel)(call.GetArguments()[0] ?? LogLevel.None) == logLevel)
            .Where(call =>
                (call.GetArguments()[2]?.ToString() ?? string.Empty).Contains(message, StringComparison.Ordinal))
            .Where(call => call.GetArguments()[3] is TException)
            .ToList();
    ...
    public static void VerifyLogDoesNotContain(this ILogger logger, LogLevel logLevel, string message)
    {
        VerifyLogContains(logger, logLevel, message, 0);
    }
    ...
    public static void VerifyLogContains(this ILogger logger, LogLevel logLevel, string message, int expectedCount)
    {
        ...
        var matchingCalls = receivedCalls
